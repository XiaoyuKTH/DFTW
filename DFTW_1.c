	...
	int DFT1(int idft, double* xr, double* xi, double* Xr_o, double* Xi_o, int N);
	...
	int main(int argc, char* argv[]){
	  ...
	  // DFT
	  int idft = 1;
	  DFT1(idft,xr,xi,Xr_o,Xi_o,N);
	  // IDFT
	  idft = -1;
	  DFT1(idft,Xr_o,Xi_o,xr_check,xi_check,N);
	  // stop timer
	  double run_time = omp_get_wtime() - start_time;
	  printf("Threads %02i, DFTW computation in %f seconds\n", Threads, run_time);
	...
	int DFT1(int idft, double* xr, double* xi, double* Xr_o, double* Xi_o, int N){
         ...
         int Threads;
          ...
          typedef struct{double Xr; double Xi; char pad[112];} tvals;
          tvals sum[Threads];
	  ...
	  for(int k=0; k<N; k++){
	      ...
	      for(int n=0; n<N; n++){
	        // Real part of X[k]
	        sum[id].Xr += xr[n] * cos(n*k*PI2/N) + idft*xi[n] * sin(n*k*PI2/N);
	        // Imaginary part of X[k]
	        sum[id].Xi += -idft*xr[n] * sin(n*k*PI2/N) + xi[n] * cos(n*k*PI2/N);
              }
            }
